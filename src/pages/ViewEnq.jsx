...
const ViewEnq = () => {
  ...
  const { enqName, enqMobile, enqEmail, enqComment, enqStatus, loading, error } = useSelector((state) => state.enquiry);

  const [status, setStatus] = useState(enqStatus || "Submitted");
  ...
  const setEnquiryStatus = (newStatus, id) => {
    const data = { id, enqData: newStatus };
    dispatch(updateAEnquiry(data))
      .then(() => {
        message.success("Enquiry status updated successfully!");
        dispatch(resetState());
        dispatch(getAEnquiry(getEnqId)); // Refresh the enquiry data
      })
      .catch(() => {
        ...
        <div className="d-flex align-items-center gap-3">
          ...
          <div>
            <select
              ...
              onBlur={() => setEnquiryStatus(status, getEnqId)}
            >
